...
source.gpkg: source.osm
	...
	ogr2ogr -f "GPKG" -update source.gpkg source.gpkg -dialect OGRSQL -sql "SELECT source.district AS district, source.osm_id AS osm_id, full.electoral_district AS electoral_district FROM source JOIN 'full.csv'.full AS full ON source.district = full.district" -nln source_join
	...
	ogr2ogr -f "GPKG" -update source.gpkg source.gpkg -dialect OGRSQL -sql "SELECT district FROM source_join WHERE electoral_district IS NULL" -nln part
	...
	ogr2ogr -f "GPKG" -append source.gpkg source.gpkg -dialect SQLITE -sql "SELECT ST_Intersection(A.geom, B.geom) AS geom, A.*, B.inside AS electoral_district FROM part A, splits B WHERE ST_Intersects(A.geom, B.geom) AND A.district=B.district" -nlt MULTIPOLYGON -skipfailures -nln electoral_districts

	ogr2ogr -f "GPKG" -append source.gpkg source.gpkg -dialect SQLITE -sql "SELECT ST_Difference(A.geom, B.geom) AS geom, A.*, B.outside AS electoral_district FROM part A, splits B WHERE A.geom != B.geom AND A.district=B.district" -nlt MULTIPOLYGON -nln electoral_districts
	...
	rm -f electoral_district.geojson
	ogr2ogr -f GeoJSON electoral_districts.geojson source.gpkg -dialect OGRSQL -sql "SELECT electoral_districts_diss.electoral_district AS electoral_district, data.* FROM electoral_districts_diss LEFT JOIN 'data.csv'.data ON electoral_districts_diss.electoral_district = data.electoral_district"
